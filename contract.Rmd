...
params:
  weekonetuesday: "2026-01-13"
  thesisdeadline: "2026-01-23"
  springbreak: "2023-01-10"
  name: Charlize Ezernack
  labs: 7
  portfolios: 6
  grade:
    label: Grade
...
1) Earn “Satisfactory” on `r params$labs` self-paced lab assignments on the schedule specified below.
	-  Satisfactory defined:
  	...
  	- I put my full effort into completing the assignment that shows my thoughtful commitment to producing code through watching the modules ahead of class, and asking curious questions in class. 
...
2) Produce at least `r params$portfolios` “Satisfactory” portfolio pieces on the schedule specified below.
    - portfolios are demonstrative of my curiosity as a learner;
    - portfolio pieces are organized and reflective of my diverse skill set for potential employers to view.
...
- [ ] `r advdate(dayone, 1, topic)`
  - Finish Lab 1 by `r startweek`: Hello R! `r advdate(dayone, 1, assignment=startweek)`
- [ ] `r advdate(dayone, 2, topic)`
  - Finish Lab 2 by `r startweek`: Global Plastic Waste `r advdate(dayone, 2, assignment=startweek)`
  - Propose your contract to Mason by `r startweek`:  `r advdate(dayone, 2, assignment=startweek)`
- [ ]  `r advdate(dayone, 3, topic)`
  - Finish Lab 3  by `r startweek` 1/30: Nobel laureates `r advdate(dayone, 3, assignment=startweek)`
  - Finish portfolio piece 1 by `r startweek` 1/30:  `r advdate(dayone, 3, assignment=startweek)`
- [ ] `r advdate(dayone, 4, topic)`
  - Finish Lab 4  by `r startweek` 2/6: Visualizing spatial data `r advdate(dayone, 4, assignment=startweek)`
  - Finish portfolio piece 2 by `r startweek` 2/6: `r advdate(dayone, 5, assignment=startweek)`
- [ ] `r advdate(dayone, 5, topic)`
  - Finish Lab 5  by `r startweek` 2/13: Wrangling spatial data `r advdate(dayone, 5, assignment=startweek)`
  - Finish portfolio piece 3 by `r startweek` 2/13: `r advdate(dayone, 5, assignment=startweek)`
- [ ] `r advdate(dayone, 6, topic)`
  - Finish Lab 6 by `r startweek` 2/20: Ugly charts and Simpson's paradox `r advdate(dayone, 6, assignment=startweek)`
  - Finish portfolio piece 4 by `r startweek` 2/20: `r advdate(dayone, 6, assignment=startweek)`
  - Schedule a check-in with Mason by `r startweek`:  `r advdate(dayone, 6, assignment=startweek)`
- [ ] `r advdate(dayone, 7, topic)`
  - Finish Lab 7 by `r startweek` 2/20: Better Viz `r advdate(dayone, 7, assignment=startweek)`
  - Finish portfolio piece 5 by `r startweek` 2/20:  `r advdate(dayone, 7, assignment=startweek)`
- [ ] `r advdate(dayone, 8, topic)`
  - - Finish portfolio piece 6 by end of day Thursday 2/26: `r advdate(dayone, 8, assignment=startweek)`
